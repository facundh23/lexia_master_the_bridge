...
> Es un algoritmo de fusión de rankings sin parámetros de calibración. Para cada documento, suma `1 / (k + posición)` en cada ranking (k=60 por convención). El resultado premia documentos que aparecen bien posicionados en _ambas_ búsquedas — semántica y léxica. Es más robusto que promediar scores porque los scores de distintos sistemas no son comparables entre sí.
